...
/**
 * On start of the PHP worker.
 * One worker per server process is started.
 */
...
$updates_mysql = function (int $queries = 0) use ($pool): string {
	...
	while ($query_count --) {
		$id = mt_rand(1, 10000);
		$randomNumber = mt_rand(1, 10000);
		$ret = $db->updates_test_select->execute([
			$id
		]);
		...
		$world['randomNumber'] = $randomNumber;
		$db->updates_test_update->execute([
			$randomNumber,
			$id
		]);
